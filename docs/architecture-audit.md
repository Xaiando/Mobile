...
| QF-11 | **The format registry** (F3). A format is an `ExerciseFormat` in `lib/core/questions/formats/<id>/`, registered in `appFormats`, with its view registered in `formatViewsProvider`. It declares its family, objectivity, depth and difficulty rank per direction (QF-6), how it is generated, how it is presented from a seed, and how it grades an answer: one `ItemGrade` per item it settles, always including the primary item. The validator, the generator, the planner, the practice screen and the coverage checker read the same registry, so adding a format edits no shared switch. Tests pass their own registry: a test-only two-item format runs the whole path. | Format tasks run in parallel (backlog wave plan). A registry is the one place that says which formats exist, which QF-2 left to code. |
| QF-12 | **Co-items are bonus reviews** (F3). A composite exercise grades its co-items in the same transaction as its primary item. They count as bonus reviews and take no session slot. A co-item planned later in the session leaves the queue once reviewed; one left on a learning step returns at the end, like any card (FS-11). The seed is logged for every presentation, flashcards included. | The planner chooses items; formats only choose how to ask. A co-item reviewed twice in one session would waste the learner's time. |
...
| QF-13 | **Typed recall grades names, not strings** (Q1, refines QF-4). The accepted names are every node the item's relation gives for its subject (or, reversed, its object) in any period, with their alternative names. Before comparing, `normalizeName` folds case, accents and punctuation; a leading article and the answer type's words at the end ("climate", "soil", "grape variety", in the singular or plural) are dropped from both sides, so "the maritime climate" answers *Oceanic*.<ul><li>An accepted name is Good.</li><li>The name of any other node, or an alternative name of one, is Again, even one letter from an accepted name.</li><li>One edit away from an accepted name of four letters or more is Hard.</li><li>A run of whole words of an accepted name, five letters or more, that is part of no other name, is Hard: "frost" for *Spring frost*, but not "marl" (*Tortonian marl*) or "Rhône" (*Southern Rhône*).</li><li>Anything else is Again.</li></ul>The payload records the text, the name matched and the outcome. The format ranks after flashcards (forward 6, reverse 7), so a new item is first shown its answer. Templates are forward only: a reverse typed question accepts only the subjects the dataset lists, which needs a completeness assertion (QF-8), so reverse typed recall waits for Q2. | A grader that fails a correct answer teaches the wrong lesson. Type words and parts of names are how people answer; another node's name is a confusion, not a slip. The five `mcq_disabled` items are the ones where a defensible alternative exists, so typed recall accepts only what the dataset asserts and shows the assertion when it disagrees. |
| QF-14 | **Short written answers check facts, not prose** (Q1, spec §T). A template names in `parameters.key_points` the relation types whose items are key points, each with a label; the format checks its own parameters (`templateProblems`, rule `template-parameters`). Ingestion writes one pool per subject with at least two key points in force. An exercise checks the planned item and up to three more the learner has studied, due ones first, each group in the seed's order; one new item joins only when none has been studied, so that two points are checked. A short answer thus does not introduce items past the session's budget (A-2). They are shown in the template's order, as "Soil: Chalk" with the item's statement. The learner writes, then reveals the points and ticks those the answer covered: ticked is Good, not ticked Again. The text goes into the planned item's `answer_payload` and is never graded by the app.<ul><li>Short answers are recall at depth 2 (QF-6), ranked after typed recall (8). Question-system §2's depth 4 would serve them on no track: no mapping is deeper than 3.</li><li>An item served only self-graded formats, such as the flashcard and the short answer, counts as flashcard-only (refines COV-2).</li><li>The coverage checker counts pooled formats as the planner serves them; an item a pooled format misses is "in no pool".</li></ul> | WSET Level 3 asks for short written answers. The app cannot grade prose offline, so the learner checks the answer against the facts it should contain, and each fact keeps its own memory. Four points keep the check quick and honest, and a self-graded pair must not hide an item that nothing grades objectively. |
| QF-15 | **The ladder as built** (F4, refines QF-7). `MemoryBand.of` puts an item in one of four bands: learning (new, or on a learning or relearning step), young (stability under 7 days), maturing (under 30) and mature. Each format declares the bands it is preferred in (`preferredBands`). By default:<ul><li>recognition while learning, recall when young, structured when young or maturing, reasoning when mature;</li><li>any reverse question when maturing;</li><li>a spatial format in every band, since its map mode hardens with the band (GEO-28).</li></ul>The short answer waits until maturing. A new item gets the easiest format preferred while learning, or in the nearest band that has one (FS-15). A later presentation first sets aside the last format (the same mode and direction) while another is served. One time in five it then draws among the rest; otherwise it draws among those preferred in the item's band, or the nearest band that has one, the easier band first on a tie. The draw uses the session's random source; the planner reads each item's last template from the review log, and the session updates it after each review. | The bands are the ones the map modes already used, so the whole presentation hardens together. The draw within a band keeps every preferred format in use, where the easiest-first rule would starve typed recall behind the flashcard. Each format's own declaration keeps the chooser free of format names (QF-11). |
...
| GEO-23 | **The release names the geography manifest** (G2). `curriculum.yaml` names tool/geography's manifest under `geography:`, after its `includes`, and the release checksum covers it. The loader turns each layer's `source_citation_ids` into `map_layer_citations` rows, in order. The manifest's `asset_bytes` and `attribution` are the pipeline's own records, which `npm run check` verifies, so they are not stored. The map sources are `dataset` citations in `areas/geography.yaml`, so About credits them (GEO-14). | The pipeline's output stays its own, and one ingestion path brings layers and facts in together (V-7). |
| GEO-24 | **Ingestion checks every map asset before writing** (G2). Each layer's asset must match its SHA-256 and parse as a topology, and must hold the feature of each of its geometries; otherwise nothing is written. The app reads the assets from its bundle, and `lint` runs the same check on the files. | A broken map must fail the build or the startup, never a question (geography §3). |
| GEO-25 | **A node drawn on its parent's map needs its location item** (G2, GEO-6). The validator reports as an error a `LOCATED_IN` relation in force whose two ends are both drawn when no item exists for it. G2 wrote the 14 missing items, each citing a primary legal text; the four region-in-France items have no MCQ while France and Italy are the only countries, and G4's map formats will practise them. | Every area the map draws inside another can be asked there. |
| GEO-26 | **Frames** (G2, geography §5). A question about a node is framed on its nearest drawn ancestor, grown by 10 % on each side, whose box holds at least four drawn nodes of the node's type, counted by label point. Otherwise the frame moves up a level; a node with no such ancestor gets no map question. `GeometryRepository` answers frames, candidates, siblings and layers with their composed attributions. | Four candidates mirror the MCQ's three wrong options (QG-6), and label points make the count deterministic. |
| GEO-27 | **Map formats as built** (G4). `map_locate` ("Find Chablis on the map.") and `map_identify` (a highlighted area and four names from the frame) ask a location item whose area is drawn and framed (GEO-26); `isEligible` skips the others, so an area not yet drawn keeps its text questions only. Locate grades the tapped node, or a name from the accessible list (GEO-13), and logs the tap, the view, the frame and the mode in `answer_payload`. Identify grades like an MCQ and logs its options. Both practise the location item itself (GEO-1). | Spatial practice for every drawn area, in the same memory as its text questions. |
| GEO-28 | **Modes follow memory** (G4, kept by F4). The mode follows the item's memory, on the ladder's bands (QF-15): labelled while new or on a step, outline under 7 days of stability, minimal under 30, blank beyond (the bands of QF-7). Identify never uses the labelled mode, which would name its answer. The backlog's "the depth sets them" would need the track at presentation time; the stability bands anticipate F4 instead. | Desirable difficulty as memory grows, with no second source of truth for modes. |
| GEO-29 | **A question draws its candidates' layer at any zoom** (G4). A frame that moves up a level can show a layer below its minimum zoom, as France frames the appellations at about zoom 4. The question draws that layer anyway, below its minimum zoom and above its maximum, and its small areas as markers at their label points, so every answer can be seen and tapped however far the learner zooms (geography §5). The map's source button is a full 48-pixel target. | A candidate that cannot be drawn cannot be answered. |
| GEO-30 | **A map this app cannot draw falls back to the list** (G4). An app downgrade keeps the newer curriculum it finds (`newerInstalled`), so the database can name a layer whose asset this bundle lacks or holds in an older edition. The map loader checks each asset against the SHA-256 the database holds and leaves off a layer that is missing or different. A question whose candidates' layer is left off says so, and its list (locate) or options (identify) still answer it. | A downgrade must never break practice; the next upgrade restores the map. |
